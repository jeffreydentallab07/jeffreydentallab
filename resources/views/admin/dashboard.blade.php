...
>
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        ...
        <div class="overflow-x-auto rounded-xl shadow-lg mt-4 mx-[15px] overflow-y-auto max-h-[60vh]">
           <table class="min-w-full border-separate border-spacing-0">
        <thead class="sticky top-0 bg-blue-900 text-white z-10">
            <tr>
                        <th class="px-4 py-2 text-left text-left">Clinic Name</th>
                        <th class="px-4 py-2 text-left text-left">Assigned Technician</th>
                        <th class="px-4 py-2 text-left text-left">Schedule</th>
                ...
                <tbody class="divide-y divide-gray-200 text-gray-800">
                    @forelse($recentAppointments as $appt)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-2 text-left">{{ $appt->caseOrder->clinic->clinic_name ?? 'N/A' }}</td>
                            <td class="px-4 py-2 text-left">{{ $appt->technician->name ?? 'Unassigned' }}</td>
                            <td class="px-4 py-2 text-left">{{ \Carbon\Carbon::parse($appt->schedule_datetime)->format('M j, Y - g:i A') }}</td>
                            <td class="px-4 py-2 text-left text-center">
    ...
    <div x-show="showModal === 'clinics'" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-cloak>
        ...
        <thead class="sticky top-0 bg-blue-900 text-white z-10">
            <tr>
                            <th class="px-4 py-2 text-left text-left">Clinic Name</th>
                            <th class="px-4 py-2 text-left text-left">Email</th>
                            <th class="px-4 py-2 text-left text-left">Contact</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($clinics as $clinic)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-2 text-left">{{ $clinic->clinic_name }}</td>
                                <td class="px-4 py-2 text-left">{{ $clinic->email }}</td>
                                <td class="px-4 py-2 text-left">{{ $clinic->contact_number ?? 'N/A' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    <div x-show="showModal === 'materials'" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-cloak>
       ...
       <table class="min-w-full border-separate border-spacing-0">
        <thead class="sticky top-0 bg-blue-900 text-white z-10">
            <tr>
                <th class="px-4 py-2 text-left text-left">Material Name</th>
                <th class="px-4 py-2 text-left text-left">Price</th>
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-200 text-gray-800 bg-white">
            @foreach($materials as $m)
                <tr class="hover:bg-gray-50">
                    <td class="px-4 py-2 text-left">{{ $m->name }}</td>
                    <td class="px-4 py-2 text-left">₱{{ number_format($m->price, 2) }}</td>
                </tr>
            @endforeach
                    </tbody>
                </table>
    ...
    <div x-show="showModal === 'caseOrders'" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-cloak>
        ...
        <thead class="sticky top-0 bg-blue-900 text-white z-10">
            <tr>
                            ...
                            <th class="px-4 py-2 text-left">Clinic Name </th>
                            <th class="px-4 py-2 text-left">Case Type</th>
                            <th class="px-4 py-2 text-left">Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($caseOrders as $c)
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-2 text-left">{{ $c->co_id }}</td>
                                <td class="px-4 py-2 text-left">{{ $c->clinic->clinic_name ?? 'N/A' }}</td>
                                <td class="px-4 py-2 text-left">{{ $c->case_type ?? 'N/A' }}</td>
                                <td class="px-4 py-2 text-left">
            @php
