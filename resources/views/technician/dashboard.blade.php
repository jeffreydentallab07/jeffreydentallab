...
<div class="container mx-auto mt-24 px-3 sm:px-6">
    @if($appointments->isEmpty())
        <p class="text-gray-600 text-center text-sm sm:text-base">No appointments assigned to you.</p>
    @else
        <div class="bg-white shadow-lg rounded-xl overflow-hidden">
            <div class="overflow-x-auto">
                <table class="min-w-full border border-gray-200 text-[11px] sm:text-sm md:text-base">
                    <thead class="bg-blue-900 text-white">
                        <tr>
                            <th class="px-3 sm:px-5 py-3 text-left">Appointment No.</th>
                            <th class="px-3 sm:px-5 py-3 text-left">Case Type</th>
                            <th class="px-3 sm:px-5 py-3 text-left">Note</th>
                            <th class="px-3 sm:px-5 py-3 text-left">Work Status</th>
                            <th class="px-3 sm:px-5 py-3 text-left">Material</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($appointments as $appointment)
                            ...
                            <tr class="hover:bg-blue-50 transition border-b">
                                {{-- Appointment No --}}
                                <td class="px-3 sm:px-5 py-3 text-gray-700 whitespace-nowrap">
                                    @if($appointment->caseOrder)
                                        ...
                                        <div id="modal{{ $appointment->appointment_id }}"
                                             class="hidden fixed inset-0 bg-gray-700 bg-opacity-50 flex items-center justify-center z-50 p-3">
                                            <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg sm:max-w-2xl max-h-[90vh] overflow-y-auto relative">
                                                <div class="p-4 border-b border-gray-200 flex items-center justify-between">
                                                    <div class="flex items-center space-x-4">
                                                        ...
                                                        <div>
                                                            <h2 class="text-base sm:text-lg font-semibold text-gray-800">{{ $appointment->caseOrder->clinic->clinic_name }}</h2>
                                                            <p class="text-xs sm:text-sm text-gray-500">{{ $appointment->caseOrder->clinic->address }}</p>
                                                            <p class="text-xs sm:text-sm text-gray-500">Contact: {{ $appointment->caseOrder->clinic->contact_number }}</p>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="p-4 space-y-4 text-sm text-gray-700">
                                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                        <div>
                                                            <label class="block font-medium text-gray-700 text-xs sm:text-sm">Case No.</label>
                                                            <p class="mt-1 px-2 py-1 border rounded bg-gray-100">{{ 'CASE-' . str_pad($appointment->caseOrder->co_id, 5, '0', STR_PAD_LEFT) }}</p>
                                                        </div>
                                                        <div>
                                                            <label class="block font-medium text-gray-700 text-xs sm:text-sm">Case Type</label>
                                                            <p class="mt-1 px-2 py-1 border rounded bg-gray-100">{{ $appointment->caseOrder->case_type }}</p>
                                                        </div>
                                                        <div>
                                                            <label class="block font-medium text-gray-700 text-xs sm:text-sm">Patient Name</label>
                                                            <p class="mt-1 px-2 py-1 border rounded bg-gray-100">{{ $appointment->caseOrder->patient?->patient_name ?? 'N/A' }}</p>
                                                        </div>
                                                        <div>
                                                            <label class="block font-medium text-gray-700 text-xs sm:text-sm">Dentist Name</label>
                                                            <p class="mt-1 px-2 py-1 border rounded bg-gray-100">{{ $appointment->caseOrder->patient?->dentist?->name ?? 'N/A' }}</p>
                                                        </div>
                                                        <div class="col-span-2">
                                                            <label class="block font-medium text-gray-700 text-xs sm:text-sm">Notes</label>
                                ...
                                {{-- Case Type --}}
                                <td class="px-3 sm:px-5 py-3 text-gray-700">{{ $appointment->caseOrder?->case_type ?? 'N/A' }}</td>

                                {{-- Note --}}
                                <td class="px-3 sm:px-5 py-3 text-gray-700">
                                    <div class="px-2 py-1 border rounded bg-gray-100 whitespace-pre-line max-w-[100px] sm:max-w-[200px] md:max-w-xs max-h-16 overflow-auto">
                                        {{ $appointment->caseOrder?->notes ?? 'N/A' }}
                                    </div>
                                </td>

                                {{-- Work Status --}}
                                <td class="px-3 sm:px-5 py-3 text-gray-700 text-center">
                                    @if($isFinished)
                                ...
                                <td class="px-3 sm:px-5 py-3 text-gray-700 text-center">
                                    <form id="materialForm_{{ $appointment->appointment_id }}" 
                                        ...
                                        <select name="material_id"
                                                class="border border-gray-300 rounded px-2 py-1 text-xs sm:text-sm focus:ring-blue-900 focus:border-blue-900 bg-white w-24 sm:w-32
