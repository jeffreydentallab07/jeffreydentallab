  ...
  <div class="w-full max-w-4xl h-[600px] bg-white rounded-2xl shadow-lg overflow-hidden flex">
    ...
    <div class="w-1/2 p-6 flex flex-col justify-center bg-white overflow-y-auto">
      ...
      <form id="clinicSignupForm" action="{{ route('clinic.signup') }}" method="POST" class="space-y-3">
        ...
        <p id="clinicNameError" class="text-red-600 text-sm mt-1 hidden">Clinic name can only contain letters, numbers, and spaces.</p>

        <!-- Owner Name -->
        <input type="text" name="owner_name" id="owner_name" placeholder="Owner Name"
               value="{{ old('owner_name') }}" required
               class="w-full border-b border-gray-300 p-2 focus:outline-none focus:border-[#189ab4]">
        <p id="ownerNameError" class="text-red-600 text-sm mt-1 hidden">Owner name can only contain letters and spaces.</p>
